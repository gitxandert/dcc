...
use crate::svs::tiff::{
    ByteOrder, FieldType, RawIfdEntry, TiffHeader, BIGTIFF_MAGIC, BYTE_ORDER_BE, BYTE_ORDER_LE,
    TIFF_MAGIC,
};
        ...
        match entry.field_type {
            ...
            FieldType::Short | FieldType::SShort => {
                for _ in 0..count {
                    let mut buf = [0u8; 2];
                    r.read_exact(&mut buf)?;
                    result.push(u16_from(buf, byte_order) as u32);
                }
            }
            _ => {
                ...
                for _ in 0..count {
                    let mut buf = [0u8; 4];
                    r.read_exact(&mut buf)?;
                    result.push(u32_from(buf, byte_order));
                }
            }
        }
        Ok(result)
...
/// Parse a complete Classic TIFF/SVS file into an [`SvsFile`].
///
